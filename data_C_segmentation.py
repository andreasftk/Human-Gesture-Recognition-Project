...
file_name = 'KaresiouWear_2024-06-18T17.01.24.341_F5174DCC3C91_Both.csv'
...
def classify_movement(row):
    if row['y-axis (g)'] < 0.38 and row['y-axis (g)'] > 0.23: 
        if row['energy'] > 0.0 and row['energy'] < 0.1: 
            return 0
        else:
            return 1
    else:
...
data_subset = data.iloc[:4215].reset_index(drop=True)
