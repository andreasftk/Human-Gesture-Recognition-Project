...
file_name = 'KaresiouWear_2024-06-16T18.30.41.382_F5174DCC3C91_Both.csv'
...
def classify_movement(row):
    if row['y-axis (g)'] < 0.32 and row['y-axis (g)'] > 0.25: 
        if row['energy'] < 0.35 and row['energy'] > 0.2: 
            return 0
        else:
            return 1
    else:
...
data_subset = data.iloc[400:3000].reset_index(drop=True)
