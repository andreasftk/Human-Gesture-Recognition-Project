...
file_name = 'KaresiouWear_2024-06-16T19.01.43.026_F5174DCC3C91_Both.csv'
...
def classify_movement(row):
    if row['y-axis (deg/s)'] < 0.53 and row['y-axis (deg/s)'] > 0.47: 
        if row['y-axis (g)'] < 0.46 and row['y-axis (g)'] > 0.30:
            if row['energy'] < 0.46 and row['energy'] > 0.35:  
                return 0
            else:
                return 1
        else:
...
data_subset = data.iloc[300:2657].reset_index(drop=True)
