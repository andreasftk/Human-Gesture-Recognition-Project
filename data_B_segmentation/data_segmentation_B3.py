...
file_name = 'KaresiouWear_2024-06-16T19.10.14.554_F5174DCC3C91_Both.csv'
...
def classify_movement(row):
    if row['y-axis (deg/s)'] < 0.4 and row['y-axis (deg/s)'] > 0.32: 
        if row['y-axis (g)'] < 0.52 and row['y-axis (g)'] > 0.46:
            if row['energy'] < 0.43 and row['energy'] > 0.33:  
                return 0
            else:
                return 1
        else:
