...
file_name = 'KaresiouWear_2024-06-16T19.13.12.033_F5174DCC3C91_Both.csv'
...
def classify_movement(row):
    if row['y-axis (deg/s)'] < 0.55 and row['y-axis (deg/s)'] > 0.46: 
        if row['y-axis (g)'] < 0.5 and row['y-axis (g)'] > 0.34:
            if row['energy'] < 0.43 and row['energy'] > 0.29:  
                return 0
            else:
                return 1
        else:
