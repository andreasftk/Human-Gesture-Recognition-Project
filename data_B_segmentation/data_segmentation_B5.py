...
file_name = 'KaresiouWear_2024-06-16T19.16.37.356_F5174DCC3C91_Both.csv'
...
def classify_movement(row):
    if row['y-axis (deg/s)'] < 0.63 and row['y-axis (deg/s)'] > 0.6: 
        if row['y-axis (g)'] < 0.46 and row['y-axis (g)'] > 0.34:
            if row['energy'] < 0.36 and row['energy'] > 0.28:  
                return 0
            else:
                return 1
        else:
