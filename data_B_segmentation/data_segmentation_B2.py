...
file_name = 'KaresiouWear_2024-06-16T19.06.09.303_F5174DCC3C91_Both.csv'
...
def classify_movement(row):
    if row['y-axis (deg/s)'] < 0.65 and row['y-axis (deg/s)'] > 0.55: 
        if row['y-axis (g)'] < 0.42 and row['y-axis (g)'] > 0.3:
            if row['energy'] < 0.33 and row['energy'] > 0.250:  
                return 0
            else:
                return 1
        else:
